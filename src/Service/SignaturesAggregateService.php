...
namespace BlackCat\Database\Packages\Signatures\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\Signatures\Dto\SignatureDto;
use BlackCat\Database\Packages\Signatures\Mapper\SignatureDtoMapper;
 ...
 */
final class SignaturesAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private SignatureRepository $signatureRepo
    ) {
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\Signatures\ModuleException("Record {$idStr} not found for locking.");
            }
