...
use BlackCat\Database\Packages\Signatures\Repository\SignatureRepositoryInterface;
...
class SignatureRepository implements SignatureRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\Signatures\Definitions::class);
    ...
    public function getBySubjectTableAndSubjectPkAndContextAndAlgoId(string $subjectTable, string $subjectPk, string $context, int $algoId, bool $asDto = false): array|\BlackCat\Database\Packages\Signatures\Dto\SignatureDto|null {
        return $this->getByUnique([ 'subject_table' => $subjectTable, 'subject_pk' => $subjectPk, 'context' => $context, 'algo_id' => $algoId ], $asDto);
    }
    ...
    public function getIdBySubjectTableAndSubjectPkAndContextAndAlgoId(string $subjectTable, string $subjectPk, string $context, int $algoId) {
        $row = $this->getBySubjectTableAndSubjectPkAndContextAndAlgoId($subjectTable, $subjectPk, $context, $algoId, false);
