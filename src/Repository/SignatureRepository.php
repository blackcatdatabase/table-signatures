...
namespace BlackCat\Database\Packages\Signatures\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\Signatures\Definitions;
use BlackCat\Database\Packages\Signatures\Criteria;
use BlackCat\Database\Packages\Signatures\Dto\SignatureDto as Dto;
use BlackCat\Database\Packages\Signatures\Mapper\SignatureDtoMapper as RowMapper;
...
final class SignatureRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\Signatures\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\Signatures\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Signatures\Dto\SignatureDto|null */
    public function getBySubjectTableAndSubjectPkAndContextAndAlgoId(string $subjectTable, string $subjectPk, string $context, int $algoId, bool $asDto = false): array|\BlackCat\Database\Packages\Signatures\Dto\SignatureDto|null {
        $row = $this->getByUnique([ 'subject_table' => $subjectTable, 'subject_pk' => $subjectPk, 'context' => $context, 'algo_id' => $algoId ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\Signatures\Mapper\SignatureDtoMapper::fromRow($row);
    }
    public function existsBySubjectTableAndSubjectPkAndContextAndAlgoId(string $subjectTable, string $subjectPk, string $context, int $algoId): bool {
        $where = 't.' . Ident::q($this->db, 'subject_table') . ' = :uniq_subject_table' . ' AND ' . 't.' . Ident::q($this->db, 'subject_pk') . ' = :uniq_subject_pk' . ' AND ' . 't.' . Ident::q($this->db, 'context') . ' = :uniq_context' . ' AND ' . 't.' . Ident::q($this->db, 'algo_id') . ' = :uniq_algo_id';
        return $this->exists($where, [ 'uniq_subject_table' => $subjectTable, 'uniq_subject_pk' => $subjectPk, 'uniq_context' => $context, 'uniq_algo_id' => $algoId ]);
    }
    /** @return int|string|null */
    public function getIdBySubjectTableAndSubjectPkAndContextAndAlgoId(string $subjectTable, string $subjectPk, string $context, int $algoId) {
        $row = $this->getBySubjectTableAndSubjectPkAndContextAndAlgoId($subjectTable, $subjectPk, $context, $algoId);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Signatures\Dto\SignatureDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\Signatures\Dto\SignatureDto|null {
