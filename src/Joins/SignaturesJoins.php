...
namespace BlackCat\Database\Packages\Signatures\Joins;
...
final class SignaturesJoins {
    ...
    /**
     * FK: signatures -> crypto_algorithms
    ...
    /**
     * FK: signatures -> crypto_algorithms
     * LEFT JOIN vw_crypto_algorithms AS $as ON $as.id = $alias.hash_algo_id
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinCryptoAlgorithmsByHashAlgoId(string $alias = 't', string $as = 'j1'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_crypto_algorithms AS ' . $as . ' ON ' . $as . '.id = ' . $alias . '.hash_algo_id' . ' ', []];
    }
    /**
     * FK: signatures -> signing_keys
     * LEFT JOIN vw_signing_keys AS $as ON $as.id = $alias.signing_key_id
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinSigningKeys(string $alias = 't', string $as = 'j2'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
