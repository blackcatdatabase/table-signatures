...
final class Criteria extends BaseCriteria
{
    ...
    {
        return [ 'id', 'subject_table', 'subject_pk', 'context', 'algo_id', 'signing_key_id', 'signature', 'payload_hash', 'hash_algo_id', 'created_at' ];
    }

    /** Columns used for full-text LIKE/ILIKE searches. */
    protected function searchable(): array
    {
        return [ 'subject_table', 'subject_pk', 'context' ];
    }

    /** Columns allowed in ORDER BY (falls back to filterable() when empty). */
    protected function sortable(): array
    {
        return [ 'id', 'subject_table', 'subject_pk', 'context', 'algo_id', 'signing_key_id', 'hash_algo_id', 'created_at' ];
    }
    ...
    protected function joinable(): array
    {
        ...
        return [ 'crypto_algorithms' => 'j0', 'signing_keys' => 'j1' ];
    }
    ...
    public static function fromDb(
        ...
        if (\method_exists(\BlackCat\Database\Packages\Signatures\Definitions::class, 'softDeleteColumn')) {
            $soft = \BlackCat\Database\Packages\Signatures\Definitions::softDeleteColumn();
